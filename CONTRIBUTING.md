...
If you want to use Android Studio or Xcode to edit the native code, you can open the `example/android` or `example/ios` directories respectively in those editors. To edit the Objective-C or Swift files, open `example/ios/ExpoDocumentScannerExample.xcworkspace` in Xcode and find the source files at `Pods > Development Pods > expo-document-scanner`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `expo-document-scanner` under `Android`.
...
```sh
Running "ExpoDocumentScannerExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
